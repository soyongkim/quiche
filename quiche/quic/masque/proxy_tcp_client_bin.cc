...
class MasqueTlsTcpClientHandler : public ConnectingClientSocket::AsyncVisitor,
 ...
 public:
  ...
  bool Start() {
    ...
    connect_socket_ = socket_factory_->CreateTcpClientSocket(
      socket_address_, /*receive_buffer_size=*/0, /*send_buffer_size=*/0,
        /*async_visitor=*/this);
  ...
  void ConnectComplete(absl::Status status) override {
    ...
    std::cout << "TCP connected to " << socket_address_ << std::endl;

    if(current_socket_type_ == SocketType::KConnectSocket) {
      std::cout << "[SD] Send CONNECT to proxy" << std::endl;
      MaybeConnectAndRequest();
      current_socket_type_ = SocketType::KConnectSocket;
  ...
  void ReceiveComplete(absl::StatusOr<quiche::QuicheMemSlice> data) override {
    ...
    if (current_socket_type_ == SocketType::KConnectSocket) {
      ...
      if (response.find("200 Connection established") != std::string::npos) {
        ...
        current_socket_type_ = SocketType::kMainSocket;
        ...
        if (ret != 1) {
          ...
          if (ssl_err == SSL_ERROR_WANT_READ) {
            ...
            //std::cout << "SSL_connect will require another read" << std::endl;
            SendToTransport();
            connect_socket_->ReceiveAsync(kBioBufferSize);
            return;
          }
    ...
    while (true) {
      ...
      if (ssl_read_ret == 0) {
        ...
        std::cout << "TLS read closed" << std::endl;
        done_ = true;
        return;
      }
      if (!h2_selected_) {
  ...
  void SendComplete(absl::Status status) override {
    ...
    if(current_socket_type_ == SocketType::kMainSocket) {
      SendToTransport();
    }
  }
 ...
 private:
  ...
  void MaybeConnectAndRequest() {
    if(request_sent_ || done_) {
      return;
    }

    if (quiche::GetQuicheCommandLineFlag(FLAGS_connect_target).empty()) {
      QUICHE_LOG(ERROR) << "No connect target specified";
      done_ = true;
      return;
    }

    SendH1ConnectRequest();
  }
  ...
  void SendToTransport() {
    ...
    if (read_ret == 0) {
      ...
      std::cout << "[SD] TLS wrote " << read_ret << " bytes to transport" << std::endl;
      connect_socket_->SendAsync(std::string(buffer, read_ret));
    }
  }
  ...
  void SendH1Request() {
    ...
    std::string request = absl::StrCat("GET ", "/",
                                       " HTTP/1.1\r\nHost: ", connect_target,
                                       "\r\nConnection: close\r\n\r\n");
  ...
  void SendH2Request() {
    ...
    headers[":authority"] = connect_target;
    headers[":path"] = "/";
  ...
  std::unique_ptr<ConnectingClientSocket> connect_socket_;
...
int RunMasqueTcpClient(int argc, char *argv[]) {
  const char *usage = "Usage: proxy_tcp_client <url>";
